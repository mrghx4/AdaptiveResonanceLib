...
BUILD_DIR="${ARTLIB_CPP_NATIVE_BUILD_DIR:-$ROOT_DIR/build/cpp-native}"
...
if ! command -v "$CMAKE_BIN" >/dev/null 2>&1; then
  echo "cmake is required for native C++ tests. Install cmake or set CMAKE=/path/to/cmake." >&2
  exit 2
fi
if ! command -v "$CTEST_BIN" >/dev/null 2>&1; then
  echo "ctest is required for native C++ tests. Install ctest or set CTEST=/path/to/ctest." >&2
  exit 2
fi

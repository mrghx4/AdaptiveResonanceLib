...
public:
    ...
    ) : params_{rho, alpha, beta, r_hat, std::move(MT), epsilon} {
    ...
    fit(XArray X, YArray y) {
        ...
        artlib_cpp::HypersphereARTMAPCore core(params_);
...
auto FitHypersphereARTMAP(
    ...
    cppHypersphereARTMAP model(
        rho, alpha, beta, r_hat, MT, epsilon, weights, cluster_labels
    );
    return model.fit(X, y);
}

auto PredictHypersphereARTMAP(
    XArray X,
    double rho,
    double alpha,
    double beta,
...
PYBIND11_MODULE(cppHypersphereARTMAP, m) {
    py::class_<cppHypersphereARTMAP>(m, "cppHypersphereARTMAP")
        .def(
            py::init<double, double, double, double, std::string, double, py::object, py::object>(),
            py::arg("rho"),
            py::arg("alpha"),
            py::arg("beta"),
            py::arg("r_hat"),

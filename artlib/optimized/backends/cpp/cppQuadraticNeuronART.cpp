...
class cppQuadraticNeuronART {
public:
    cppQuadraticNeuronART(
        double rho,
        double s_init,
        double lr_b,
        double lr_w,
    ...
    ) : params_{rho, s_init, lr_b, lr_w, lr_s} {
        if (!weights.is_none()) {
    ...
    std::tuple<py::array_t<int>, std::vector<py::array_t<double>>> fit(XArray X) {
        ...
        artlib_cpp::QuadraticNeuronARTCore core(params_);
...
private:
    artlib_cpp::QuadraticNeuronARTParams params_;
    std::vector<std::vector<double>> initial_weights_;
};

auto FitQuadraticNeuronART(
    XArray X,
    double rho,
    double s_init,
    double lr_b,
    ...
    cppQuadraticNeuronART model(rho, s_init, lr_b, lr_w, lr_s, weights);
    return model.fit(X);
}

auto PredictQuadraticNeuronART(
    XArray X,
    double rho,
    double s_init,
    double lr_b,
...
PYBIND11_MODULE(cppQuadraticNeuronART, m) {
    py::class_<cppQuadraticNeuronART>(m, "cppQuadraticNeuronART")
        .def(
            py::init<double, double, double, double, double, py::object>(),
            py::arg("rho"),
            py::arg("s_init"),
            py::arg("lr_b"),
            py::arg("lr_w"),
        ...
        .def("fit", &cppQuadraticNeuronART::fit, py::arg("X"))
        .def("predict", &cppQuadraticNeuronART::predict, py::arg("X"))
        .def("__repr__", [](const cppQuadraticNeuronART&) { return "<cppQuadraticNeuronART>"; });

    m.def(
        "FitQuadraticNeuronART",
        &FitQuadraticNeuronART,
        py::arg("X"),
        py::arg("rho"),
        py::arg("s_init"),

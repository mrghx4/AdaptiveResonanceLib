...
public:
    ...
    std::tuple<py::array_t<int>, std::vector<py::array_t<double>>> fit(XArray X) {
        ...
        artlib_cpp::ART2Core core(params_);
...
auto FitART2(
    ...
    cppART2 model(rho, alpha, beta, weights);
    return model.fit(X);
}
...
PYBIND11_MODULE(cppART2, m) {
    py::class_<cppART2>(m, "cppART2")
        .def(
        ...
        .def("fit", &cppART2::fit, py::arg("X"))
        .def("predict", &cppART2::predict, py::arg("X"))
        .def("__repr__", [](const cppART2&) { return "<cppART2>"; });

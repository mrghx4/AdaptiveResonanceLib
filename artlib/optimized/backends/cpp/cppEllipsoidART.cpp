...
public:
    ...
    ) : params_{rho, alpha, beta, mu, r_hat} {
        if (!weights.is_none()) {
    ...
    std::tuple<py::array_t<int>, std::vector<py::array_t<double>>> fit(XArray X) {
        ...
        artlib_cpp::EllipsoidARTCore core(params_);
...
private:
    artlib_cpp::EllipsoidARTParams params_;
    std::vector<std::vector<double>> initial_weights_;
};

auto FitEllipsoidART(
    XArray X,
    double rho,
    double alpha,
    double beta,
    ...
    cppEllipsoidART model(rho, alpha, beta, mu, r_hat, weights);
    return model.fit(X);
}

auto PredictEllipsoidART(
    XArray X,
    double rho,
    double alpha,
    double beta,
...
PYBIND11_MODULE(cppEllipsoidART, m) {
    py::class_<cppEllipsoidART>(m, "cppEllipsoidART")
        .def(
        ...
        .def("fit", &cppEllipsoidART::fit, py::arg("X"))
        .def("predict", &cppEllipsoidART::predict, py::arg("X"))
        .def("__repr__", [](const cppEllipsoidART&) { return "<cppEllipsoidART>"; });

...
public:
    ...
    ) : params_{rho, alpha, parse_sigma_init(sigma_init), std::move(MT), epsilon} {
    ...
    fit(XArray X, YArray y) {
        ...
        artlib_cpp::GaussianARTMAPCore core(params_);
...
auto FitGaussianARTMAP(
    ...
    cppGaussianARTMAP model(rho, alpha, sigma_init, MT, epsilon, weights, cluster_labels);
    return model.fit(X, y);
}

auto PredictGaussianARTMAP(
    XArray X,
    double rho,
    double alpha,
    XArray sigma_init,
...
PYBIND11_MODULE(cppGaussianARTMAP, m) {
    py::class_<cppGaussianARTMAP>(m, "cppGaussianARTMAP")
        .def(
            py::init<double, double, XArray, std::string, double, py::object, py::object>(),
            py::arg("rho"),
            py::arg("alpha"),
            py::arg("sigma_init"),
            py::arg("MT"),
